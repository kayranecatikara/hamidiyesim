...
#  ELİPS GÖREVİ
# ═══════════════════════════════════════════════════════════════════
# Referans görsel: 224 x 98 m elips, sabit 17.99 m/s, 5.4 tur, 2860 m.
# Elipste en dar viraj uzun eksenin UCLARINDA: R_min = b^2/a = 21.4 m.
# 18 m/s'de bu 57 derece yatis ister; ROLL_LIMIT_DEG = 45 oldugu icin sekil
# bu hizda CIZILEMEZ. Uc kol bunun uc ayri cozumunu sinar.
...
def _param_yaz(conn, ad, deger, timeout=3.0):
    """Canli PARAM_SET + teyit. Donus: eski deger."""
...
def elips_ciz(conn, a, b, tur, hiz_hedef, bank, irtifa_hedef, ileri,
              roll_limit_deg=45.0):
    """Carrot takibi + gaz ile HIZ, burun ile IRTIFA denetimi.

    Gaz ve burun ayri organlari surer (iki dongu ayni organi cekmesin —
    inis kodundaki salinim dersi).
    """
    _pump(conn)
    yaw = _att["yaw"]
    # UZUN EKSEN KUZEY-GUNEY'E CIVILI (kullanici karari): desen, ucagin kalkis
    # yonune gore donmez — her ucusta ayni yonelimde cizilir. (Onceki hali
    # ekseni yaw'a oturtuyordu; olculdu: uzun eksen kuzeyden 158 derece sapti.)
    u = (1.0, 0.0)                                   # kuzey
    w = (0.0, 1.0)                                   # dogu (u'nun sagi)
    P0 = (_pos["x"], _pos["y"])
    # GIRIS NOKTASI: tegeti ucagin burnuna en yakin olan parametre secilir ki
    # desene sarsintisiz girsin (yoksa ters yondeyse genis bir yakalama turu
    # atmasi gerekir).
    t_bas, _fark = 0.0, 9e9
    for _k in range(720):
    ...
    if sonsuz:
        print("[ELIPS] SONSUZ desen — durdurulana kadar elips cizilir, INIS YOK")
    t0, son_bilgi = time.time(), 0.0
    sapmalar, hizlar, irtifalar = [], [], []
    gaz_i = 0.0                      # hiz integrali (kalici hatayi kapatir)
    while not _abort and t < t_son and time.time() - t0 < zaman_asimi:
        _pump(conn)
        hiz, irtifa = _yer_hizi(), -_pos["z"]
        # UCAGIN KONUMUNU ELIPSE IZDUSUR (yerel arama, geri gitmez):
        # parametreyi hizla entegre etmek gecikme biriktiriyordu (olculdu:
        # sapma 35 m'de sabitlendi, tur suresi 71 s / olmasi gereken 37 s).
        en_iyi, en_uz = t, 1e18
        ...
        # ── HAVUC: hiza gore ileri bak (sabit 25 m, 18 m/s'de 1.4 s = cok gec) ──
        ...
        # ── YATIS ON BESLEME: yerel egrilik yaricapinin GEREKTIRDIGI yatis ──
        # Yalniz hata-orantili surus, kavisli hatti yapisal olarak gecikmeyle
        # takip eder (olculdu: sapma 7 m, yatis +56/-27 salinimi).
        # ON BESLEME ILERIDEN: egrilik uclarda 256 m'den 21 m'ye coküyor;
        # bulundugu noktadan hesaplanınca ucak yatısa GEC giriyor ve disari
        # tasiyordu (olculdu: kutu 229x125, hedef 224x98).
        ...
        # ── BURUN: irtifa + YUK FAKTORU on beslemesi (yatista tasima kaybi) ──
        ...
        if time.time() - son_bilgi > 6.0:
            son_bilgi = time.time()
            _gecen = (t - t_bas) / (2 * math.pi)
            _tur_et = ("tur %5.2f" % _gecen) if sonsuz else \
                      ("tur %4.2f/%.1f" % (_gecen, tur))
            print(f"[ELIPS] {_tur_et} | hiz {hiz:5.2f} | "
                  f"irtifa {irtifa:5.1f} | yatis {math.degrees(_att['roll']):+5.1f} | "
                  f"sapma {uz:4.1f} m")
        time.sleep(CONTROL_RATE)
    n = len(sapmalar) or 1
    print(f"[ELIPS] DESEN BITTI — {(t-t_bas)/(2*math.pi):.2f} tur, {time.time()-t0:.0f} s")
    print(f"[ELIPS] OLCUM: sapma ort {sum(sapmalar)/n:.1f} m / tepe {max(sapmalar or [0]):.1f} m | "
          f"hiz ort {sum(hizlar)/n:.2f} m/s (yayilim "
          f"{max(hizlar or [0])-min(hizlar or [0]):.2f}) | "
          f"irtifa {min(irtifalar or [0]):.1f}-{max(irtifalar or [0]):.1f} m")


def scenario_elips_gorev(conn):
    """Kalkis (main'de) -> elips deseni -> firlatma noktasina inis."""
    eski_roll = None
    if _ELIPS_ROLL_LIM:
    ...
    # INIS YOK (kullanici karari): bu gorev gorsel gudum testi icin hedefi
    # havada tutar. Desen ancak durdurulunca biter; ardindan main() yuzeyleri
    # notrleyip cruise gazla birakir, manuel/baska senaryo devralabilir.
