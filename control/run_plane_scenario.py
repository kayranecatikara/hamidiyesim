...
_pos = {"x": 0.0, "y": 0.0, "z": 0.0,      # x=kuzey, y=doğu (NED, m)
        ...
        "t": 0.0}                          # son mesaj zamanı (tazelik kapısı)

# ⭐ KALKIŞ → KARE → İNİŞ (kare_gorev) İÇİN TAŞINDI — 2026-08-23,
# kalkis_kare_inis dalı (commit b9db632, Ayşenur Bayrakdar).
# Aşağıdakilerin tamamı YENİ; mevcut senaryolardan hiçbiri okumuyor.
_hb = {"armed": False, "mode": None, "ok": False}   # iniş sonrası disarm teyidi
_kalkis_xy = None            # fırlatma noktası (yerel NED); kaynağı _ev_noktasi()
_gpos = {"lat": 0.0, "lon": 0.0, "t": 0.0}   # HOME'u yerele taşımak için gerekli
_home = {"lat": 0.0, "lon": 0.0, "t": 0.0}
_M_PER_DEG = 111319.4907     # metre / enlem derecesi
_EV_KAYNAK  = os.environ.get("AVCI_EV_KAYNAK", "home")   # "home" | "yerel"
_EV_TIMEOUT = float(os.environ.get("AVCI_EV_TIMEOUT", "8.0"))
# Dönüş bacağı olan görevler: fırlatma noktası bilinmiyorsa BAŞLATILMAZ.
...
def _pump(conn):
    ...
    while True:
        ...
        # ⭐ kare_gorev için (2026-08-23): fırlatma noktası ve disarm teyidi.
...
def takeoff(conn, climb_time=8.0, hedef_alt=None):
    ...
    ⭐ hedef_alt (2026-08-23, kalkis_kare_inis dalından): verilirse sabit
    süre yerine O İRTİFAYA ULAŞANA KADAR (ya da en az 30 s üst zaman aşımına)
    TAKEOFF'ta kalınır. Dalda ölçülen sebep: sabit climb_time=8 s ile uçak
    yerde ~4 s hızlanma yiyor, yalnız ~4.5 m'ye çıkabiliyor ve FBWA + bekleme
    dairesi o irtifada banklayınca TAKLA ATIYOR (3/3 kaza, roll→55°).

    ⚠ GERİYE UYUMLULUK — DALDAN BİLEREK SAPILDI (§5.10 yapısal garanti):
    dalda `hedef_alt=None` iken AVCI_TKOFF_HEDEF=30 m'ye düşüyor, yani TÜM
    senaryoların kalkışı değişiyordu. Burada `None` = ESKİ DAVRANIŞ (sabit
    climb_time). Böylece `duz/square/circle/aggressive/elips_gorev` bit bit
    aynı kalır; yalnız `kare_gorev` açık değer geçer (bkz. _kalkis_hedefi).
    Mevcut kampanya ölçümleriyle kıyaslanabilirlik korunur (§4 tek değişken).
    """
    ...
    while not _abort and time.time() - t0 < ust_sure:
        _pump(conn)
        if hedef_alt is not None and -_pos["z"] >= hedef_alt:
            break
        time.sleep(0.2)
...
SCENARIOS = {
    ...
    # ⭐ 2026-08-23: SONLU görev — kalkış → N kare turu → fırlatma noktasına
    # iniş. `square` (süresiz, kampanya senaryosu) DEĞİŞTİRİLMEDİ.
...
def _kalkis_hedefi(name):
    """Yalnız kare_gorev açık irtifa hedefi verir; diğerleri ESKİ davranışta
    (None → sabit climb_time) kalır. Bkz. takeoff() geriye uyumluluk notu."""
...
def main():
    ...
    # ⭐ FIRLATMA NOKTASI (2026-08-23) — kaynağı _ev_noktasi(), varsayılan
    # otopilotun HOME'u. C KAPISI: belirlenemezse DÖNÜŞ BACAĞI OLAN görev
    # başlatılmaz; eski sürüm sessizce (0,0)'a düşüp uçağı boot noktasına
    # gönderiyordu. Dönüş bacağı olmayan senaryolar bundan ETKİLENMEZ.
    # ⚠ DALDAN SAPMA (§5.10 yapısal garanti): dalda _ev_noktasi() KOŞULSUZ
    # çağrılıyor ve her senaryonun açılışına MAV_CMD_REQUEST_MESSAGE +
    # GET_HOME_POSITION ekliyordu. `_kalkis_xy`'yi yalnız dönüş bacağı olan
    # görev okuduğu için burada KOŞULA bağlandı: `duz/square/circle/
    # aggressive/elips_gorev` açılışı bit bit eskisi gibi kalır.
    global _kalkis_xy
    _kalkis_xy = _ev_noktasi(conn) if name in _DONUS_GEREKTIREN else None
    ...
    # ⭐ 2026-08-23: iniş sonrası (araç YERDE ve DISARM) cruise gaz VERİLMEZ —
    # yeniden armlanırsa uçak kendi kendine yerde koşmaya başlar.
    _pump(conn)
