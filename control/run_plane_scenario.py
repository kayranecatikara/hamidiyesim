...
Kullanım:
    ...
    python -m control.run_plane_scenario kare_gorev  # kalkış → kare → iniş (biter)
...
def takeoff(conn, climb_time=8.0, hedef_alt=None):
    ...
    hedef_alt VERİLİRSE (kare_gorev) ölçüt süre değil İRTİFA olur: uçak o
    irtifaya çıkana ya da üst-zaman aşımına (climb_time'ın en az 30 s'i) kadar
    TAKEOFF'ta kalır. Verilmezse eski davranış birebir korunur — kampanya
    senaryolarının (duz/square/circle/aggressive) ölçüm geçmişi bozulmasın."""
    ust_sure = climb_time if hedef_alt is None else max(climb_time, 30.0)
    print("[SCN] Otonom kalkış (TAKEOFF modu)..." if hedef_alt is None else
          f"[SCN] Otonom kalkış (TAKEOFF modu, hedef ~{hedef_alt:.0f} m)...")
...
def _kalkis_hedefi(name):
    """kare_gorev alçak desende uçar; diğer senaryolar eski kalkışta kalır."""
...
def main():
    ...
    # YALNIZ dönüş bacağı olan görevlerde çağrılır: _ev_noktasi HOME tazeliğini
    # beklediği için kalkışı 8 s'ye kadar geciktirebilir ve kampanya
    # senaryolarının (duz/square/circle/aggressive/elips) zamanlaması bozulur.
    global _kalkis_xy
    if name in _DONUS_GEREKTIREN:
