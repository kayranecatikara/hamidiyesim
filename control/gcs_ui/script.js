...
document.getElementById('btn-plane-square').addEventListener('click', () => {
    sendCommand('plane/square', 'Otonom Uçuş (stabil LOITER daire)');
...
btnManual.addEventListener('click', async () => {
    if (!manualActive) {
        // Manuel mod AÇ (KARE ikonu korunur — yazı basma)
        ...
        try {
            ...
            if (data.status === 'success') {
                ...
                btnManual.classList.add('active-scn');        // KARE korunur, aktif vurgusu
        ...
        btnManual.classList.remove('active-scn');     // KARE ikonu kalır
...
const btnPlaneRandom = document.getElementById('btn-plane-random');

async function togglePlaneRandom() {
    if (!planeRandomActive) {
        addLog('SYS', '🎲 Hedef İHA rastgele/kaçış uçuşu başlatılıyor...', 'warn');
        try {
            ...
            if (data.status === 'success') {
                ...
                if (btnPlaneRandom) {
                    btnPlaneRandom.classList.add('active-scn');   // ÜÇGEN korunur, aktif vurgusu
                    btnPlaneRandom.style.borderLeftColor = 'var(--danger-red)';
                }
                const st = document.getElementById('plane-random-status');
                if (st) { st.textContent = 'AKTİF'; st.className = 'val green'; }
                addLog('SYS', '🎲 Hedef İHA rastgele uçuyor! (R ile durdur)', 'success');
            } else {
                addLog('ERR', 'Başlatılamadı: ' + (data.message || ''), 'crit');
            }
        } catch (e) {
        ...
        if (btnPlaneRandom) {
            btnPlaneRandom.classList.remove('active-scn');    // ÜÇGEN ikonu kalır
            btnPlaneRandom.style.borderLeftColor = '';
        }
        const st = document.getElementById('plane-random-status');
        if (st) { st.textContent = 'BEKLEMEDE'; st.className = 'val warning'; }
        addLog('SYS', 'Hedef İHA rastgele uçuş durduruldu (LOITER).', 'info');
