...
def main():
    ...
    # ══════════════════════════════════════════════════════════════════
    # Ö-D · FOV KISITI — burun komutu gövdeden fazla açılamaz
    # ══════════════════════════════════════════════════════════════════
    # Kısıt döngüde uygulanıyor; yasayı burada birebir yeniden kuruyoruz.
    def _fov(cfg, cmd_yaw_deg, iris_yaw_deg):
        """Ö-D uygulandıktan sonraki burun komutu (derece)."""
        borc = math.degrees(ib.normalize_angle(
            math.radians(cmd_yaw_deg - iris_yaw_deg)))
        if cfg.FOV_YAW_HATA > 0.0 and abs(borc) > cfg.FOV_YAW_HATA:
            borc = max(-cfg.FOV_YAW_HATA, min(cfg.FOV_YAW_HATA, borc))
        return math.degrees(ib.normalize_angle(
            math.radians(iris_yaw_deg + borc)))

    class _Fov(C):
        FOV_YAW_HATA = 25.0

    # B71 — U05 olayının ta kendisi: 40°'lik borç 25°'ye kısılır
    _u05 = _fov(_Fov, -147.0, -107.0)
    kontrol("B71 U05 olayı: 40° borç kısılır",
            abs(_u05 - (-132.0)) < 1e-6,
            f"yaw_cmd −147° / iris_yaw −107° (borç 40°) → komut {_u05:.0f}° "
            f"(borç 25°) — kutu kaybolunca ödenecek borç 40° değil 25°")

    # B72 — SAKİN TAKİPTE ÖLÜ: küçük borçta hiçbir şey değişmez (§5.10)
    _sakin = [(_fov(_Fov, a, b), a) for a, b in
              ((10.0, 0.0), (-5.0, 0.0), (100.0, 80.0), (-170.0, 175.0))]
    kontrol("B72 sakin takipte Ö-D ÖLÜ (borç < sınır → komut aynen)",
            all(abs(ib.normalize_angle(math.radians(y - a))) < 1e-9
                for y, a in _sakin),
            "borç 10°/5°/20°/15° → dördünde de komut BİT BİT aynı; "
            "normal takip etkilenmez")

    # B73 — sarma (wrap) doğru: ±180° sınırında da borç doğru hesaplanır
    _wrap = _fov(_Fov, -170.0, 175.0)
    kontrol("B73 ±180° sarmasında borç doğru",
            abs(ib.normalize_angle(math.radians(_wrap - (-170.0)))) < 1e-9,
            f"cmd −170° / iris 175° gerçek borç 15° (355° DEĞİL) → "
            f"komut {_wrap:.0f}° değişmedi")

    # B74 — YALNIZ KISAR: çıktı borcu asla büyütmez
    _buyutme = []
    for a in range(-180, 181, 15):
        for b in range(-180, 181, 45):
            once = abs(math.degrees(ib.normalize_angle(math.radians(a - b))))
            sonra = abs(math.degrees(ib.normalize_angle(
                math.radians(_fov(_Fov, float(a), float(b)) - b))))
            _buyutme.append(sonra <= once + 1e-9)
    kontrol("B74 Ö-D YALNIZ KISAR, asla büyütmez",
            all(_buyutme),
            f"{len(_buyutme)} (cmd, iris) kombinasyonunda |borç| hiç artmadı")

    # B75 — YAPISAL GARANTİ: komut() çıktısı BİT BİT aynı (uçuş yolu değişmez)
    # Ö-D döngüde, slew'den SONRA uygulanıyor; hız vektörü komut() içinde
    # `hiz_yonu`ndan hesaplanıyor ve bu sınırdan GEÇMİYOR.
    _fark, _n = 0.0, 0
    for cx in (120.0, 320.0, 520.0):
        for cy in (180.0, 300.0, 420.0):
            for bw in (12.0, 40.0, 90.0):
                for iyaw in (0.0, 1.2, -2.5):
                    for term in (False, True):
                        a = ib.komut(cx, cy, bw, bw, iyaw, 8.0, 0.05, C,
                                     term, (0.4, 0.1), 0.05, 0.0, -3.0,
                                     0.2, 0.3)
                        b = ib.komut(cx, cy, bw, bw, iyaw, 8.0, 0.05, _Fov,
                                     term, (0.4, 0.1), 0.05, 0.0, -3.0,
                                     0.2, 0.3)
                        for u, v in zip(a[:4], b[:4]):
                            _fark = max(_fark, abs(u - v)); _n += 1
    kontrol("B75 YAPISAL GARANTİ: komut() çıktısı BİT BİT aynı",
            _fark == 0.0,
            f"{_n} çıktı ({_n//4} girdi kombinasyonu) — vx, vy, vz ve yaw "
            f"hedefi hiç değişmedi (maks sapma {_fark:.2e}). "
            f"Ö-D UÇUŞ YOLUNU DEĞİŞTİREMEZ; yalnız BURUN.")

    # B76 — kapatılabilir (varsayılan KAPALI)
    kontrol("B76 Ö-D kapatılabilir (varsayılan KAPALI)",
            abs(C.FOV_YAW_HATA) < 1e-9
            and abs(_fov(C, -147.0, -107.0) - (-147.0)) < 1e-6,
            f"FOV_YAW_HATA={C.FOV_YAW_HATA} → 40°'lik borç aynen geçer "
